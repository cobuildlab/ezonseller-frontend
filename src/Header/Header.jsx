...
class Header extends React.Component {
    constructor(props) {
        ...
        this.handleHaveAccEbayAmazon = this.handleHaveAccEbayAmazon.bind(this);
    }
    ...
    componentDidMount() {
        ...
        let acc_product = valueUser.acc_product;

        if (valueUser.type_plan === 'Free' && !acc_product) {
    ...
    componentWillReceiveProps = (nextProps) => {
        ...
        if (nextProps.user && nextProps.user.items ) {
            let ebay_acc = false;
            let amazon_acc = false;
            let valueUser = JSON.parse(localStorage.getItem('user'));


            if(nextProps.user.items.amazon_account.length !== 0) {
                amazon_acc = true;
            }
            if(nextProps.user.items.ebay_account.length !== 0) {
                ebay_acc = true;
            }

            let haveAcc = this.handleHaveAccEbayAmazon(amazon_acc,ebay_acc);

            if (valueUser.type_plan !== 'Free' && !haveAcc) {
                this.setState({typePlan: true})
            }
    ...
    handleHaveAccEbayAmazon(amazon_acc,ebay_acc){
        if (amazon_acc || ebay_acc) {
            return true;
        }
                        ...
                        <div className="collapse navbar-collapse" id="navbarNav">
                            <ul className="navbar-nav hidden-md-up">
                                <li className="nav-item">
                                    <Link to="/" className="nav-link border">Home <span
                            ...
                            <div className="container">
                                <ul className="navbar-nav">
                                    <form className="form-inline hidden-sm-down"
                                          onSubmit={this.handleSearchProduct}>
                                        <li className="nav-item">
                                            {country.items &&
                                            <select className="custom-select rigth-input tam-input-nav"
                                                    id="country_id" name="country" onChange={this.handleChange}
                                                    value={this.state.country} disabled={typePlan} required>
                                                <option defaultValue="">Select a Country</option>
                                                {country.items.map((option, index) => {
                                                    return <option value={option.code}
                                                                   key={index}>{option.name} - {option.code}</option>
                                                })}
                                            </select>
                                            }
                                        </li>
                                        <li className="nav-item">
                                            <select className="custom-select rigth-input tam-input-nav"
                                                ...
                                                <option defaultValue="HealthPersonalCare">HealthPersonalCare
                                                </option>
                                                ...
                                                <option defaultValue="MusicalInstruments">MusicalInstruments
                                                </option>
                                                ...
                                                <option defaultValue="WirelessAccessories">WirelessAccessories
                                                </option>
                                            </select>
                                        </li>
                                        <li className="nav-item">
                                            <div className="form-group">
                                                <input type="text"
                                                       className="form-control rigth-input tam-input-nav"
                                                       name="keyword" id="inputPassword2"
                                                       onChange={this.handleChange} placeholder="Keywords"
                                                       disabled={typePlan}/>
                                                <button type="submit" className="btn btn-primary"
                                                        style={{'height': '50px'}} disabled={typePlan}>Search
                                                </button>
                                            </div>
                                        </li>
                                    </form>
                                        ...
                                        <li className="nav-item">
                                            <div className="form-group">
                                                ...
                                                <input type="text"
                                                       ...
                                                       onChange={this.handleChange} placeholder="Keywords"
                                                       disabled={typePlan}/>
                                                <button type="submit" className="btn btn-primary btn-block">Search
                                                </button>
                                            </div>
                                        </li>
                                    </form>
                            ...
                            <ul className="navbar-nav hidden-sm-down">
                                <li className="nav-item">
                                    <div className="dropdown">
                                        ...
                                        <button className="btn btn-link dropdown-toggle" id="dropdownMenuButton"
                                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        </button>
                                        <div className="dropdown-menu pos-menu"
